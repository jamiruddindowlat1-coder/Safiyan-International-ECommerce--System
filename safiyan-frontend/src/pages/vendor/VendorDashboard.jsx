...
export default function VendorDashboard() {
    ...
    const loadDashboard = async () => {
        try {
            ...
            const [ordersData, productsData, vendorsData] = await Promise.all([
            ...
            const myVendor = Array.isArray(vendorsData)
                ? vendorsData.find((v) => v.userId === user?.id)
                : null;

            if (myVendor) {
                try {
                    const summary = await commissionService.getVendorSummary(myVendor.id);
                    setEarnings(summary);
                } catch {
                    setEarnings(null);
                }
                ...
                {earnings ? (
                    <div style={overviewGridStyle}>
                        ...
                        <div style={overviewCardStyle}>
                            ...
                            <strong style={{ ...overviewValueStyle, color: "#b42318" }}>
                                {formatCurrency(earnings.totalCommission)}
                            </strong>
                            <span style={overviewHintStyle}>
                                {earnings.effectiveCommissionRate != null
                                    ? `${earnings.effectiveCommissionRate}% negotiated rate`
                                    : "Category-based rate"}
                            </span>
                        </div>
                        ...
                        <div style={overviewCardStyle}>
                            ...
                            <strong style={{ ...overviewValueStyle, color: "#087443" }}>
                                {formatCurrency(earnings.totalEarning)}
                            </strong>
                        </div>
                        ...
                        <div style={overviewCardStyle}>
                            ...
                            <strong style={overviewValueStyle}>
                                {formatCurrency(earnings.currentPayableBalance)}
                            </strong>
                            <span style={overviewHintStyle}>
                                Awaiting settlement from admin
                            </span>
                        </div>
                    </div>
                ) : (
                    <div style={emptyStyle}>
                        Earnings data isn't available yet.
                    </div>
                )}

                {earnings?.payouts?.length > 0 && (
                    <div style={{ ...tableWrapperStyle, marginTop: 18 }}>
                        <table style={tableStyle}>
                            ...
                            <tbody>
                                {earnings.payouts.map((p) => (
                                    <tr key={p.id}>
                                        <td style={tdStyle}>{p.payoutReference}</td>
