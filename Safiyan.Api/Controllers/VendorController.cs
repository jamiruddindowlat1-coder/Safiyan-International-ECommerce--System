...
public class VendorController : ControllerBase
{
    ...
    // PATCH: api/Vendor/5/commission-rate
    // Admin sets (or clears) a per-vendor commission rate override.
    // Passing null falls back to the vendor's category-based commission rate.
    [HttpPatch("{id:int}/commission-rate")]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> UpdateCommissionRate(int id, UpdateCommissionRateRequest request)
    {
        if (request == null)
        ...
        if (request.CommissionRateOverride.HasValue &&
            (request.CommissionRateOverride.Value < 0 || request.CommissionRateOverride.Value > 100))
        {
            return BadRequest(new { message = "Commission rate must be between 0 and 100." });
        }

        vendor.CommissionRateOverride = request.CommissionRateOverride;
        vendor.UpdatedAt = DateTime.UtcNow;
