    ...
    public async Task<ActionResult<object>> CreateOrder(CreateOrderRequest request)
    {
        ...
        foreach (var item in cart.Items)
        {
            ...
            if (!item.Product.IsActive)
                return BadRequest(new { message = $"Product '{item.Product.Name}' is no longer available." });

            if (item.Quantity > item.Product.StockQuantity)
                return BadRequest(new { message = $"Only {item.Product.StockQuantity} unit(s) left for {item.Product.Name}." });

            var unitPrice = item.Product.DiscountPrice > 0 ? item.Product.DiscountPrice : item.Product.Price;
